...
The server SHALL build voxel block shape metadata from the active save palette, layered Cubyz block definitions, block model OBJ assets, and supported block rotation metadata during startup.
...
Voxel mesh cache keys SHALL include a version or signature for block shape metadata so meshes are invalidated when shape-affecting assets or the shape payload format changes.
