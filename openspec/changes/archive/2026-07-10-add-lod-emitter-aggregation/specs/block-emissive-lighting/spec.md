...
- **WHEN** the client worker decodes a coarser LOD voxel payload with aggregated emitter records
- **THEN** it consumes those records through the optimized emissive bake path without requiring a binary decoder migration
...
#### Scenario: Coarser LOD aggregation source strategy changes
- **WHEN** aggregation changes whether it derives representatives from same-LOD data, bounded LOD 1 source data, or another source strategy
- **THEN** previously persisted coarser LOD voxel mesh cache entries generated with the old source strategy are not reused
