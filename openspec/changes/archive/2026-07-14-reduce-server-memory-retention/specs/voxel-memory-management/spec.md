...
### Requirement: Resolved emitter summary retention is byte bounded
The server SHALL bound resolved emitter-summary nodes by both entry count and the UTF-8 byte length of their serialized representation. It SHALL expose that estimated retained weight, retained cluster count, eviction and oversized-skip counts, and active summary work through voxel metrics. An individually oversized node SHALL be served without remaining in memory, and invalidated in-flight work SHALL NOT repopulate memory or persistent summary state.

#### Scenario: Summary cache exceeds its estimated-byte budget
- **WHEN** retaining a resolved emitter-summary node would exceed the configured entry or estimated-byte limit
- **THEN** the server evicts least-recently-used nodes until the cache is within both limits

#### Scenario: Oversized summary node is resolved
- **WHEN** one summary node exceeds the complete estimated-byte budget
- **THEN** the server serves the node and declines to retain it in the resolved memory cache

#### Scenario: Summary work completes after invalidation
- **WHEN** per-key or global invalidation occurs while summary work is in flight
- **THEN** the obsolete completion does not repopulate memory or persistent summary state
...
### Requirement: Memory controls preserve voxel behavior
Memory budgeting, cache eviction, and worker recycling SHALL NOT change voxel geometry, emitter results, supported LODs, coordinate interpretation, compression negotiation, or binary response semantics.

#### Scenario: Entry is evicted or its worker is recycled
- **WHEN** a subsequent request regenerates data after cache eviction or worker replacement
- **THEN** the resulting decoded voxel content is equivalent to content generated without eviction or recycling
