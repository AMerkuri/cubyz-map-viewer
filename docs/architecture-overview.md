...
- voxel vertex positions use a cache-versioned mixed encoding: ordinary greedy cube quads use compact unsigned integer cell coordinates relative to the response origin, while model/semantic quads and any non-integer geometry use unsigned fixed-point values in `1/4096` voxel-cell units; the client decodes both forms to the same world coordinate space
...
- voxel mesh payloads include compact per-quad render-kind, source-kind, and position-kind data so the client can build opaque and transparent meshes separately while preserving colors, normals, AO, winding, positions, and palette identity, and so diagnostics can distinguish greedy cube quads from model/semantic quads
