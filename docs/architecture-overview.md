...
- voxel mesh payloads include a versioned compact emitter-record section after greedy/model geometry records; LOD `1` payloads may include own-region and neighboring-region halo block emitters within the configured emitted-light radius, encoded as signed response-relative cell coordinates plus RGB bytes and an open-face mask derived from Cubyz `.emittedLight` blocks
- the server builds LOD `1` halo records with generation-local external-region and unified target/external traversability-promise caches, then applies the cache-versioned deterministic 8,192-record selection boundary: when capped, opaque quad vertices on each horizontal region edge are deduplicated into bounded four-cell Y/Z or X/Z buckets, and up to 256 protected nominations per edge are distributed across buckets among halo sources whose configured radius reaches that receiving geometry; corners participate in both edge rankings with record deduplication, while broad-volume ranking and deterministic own/halo ordering fill unused protected and payload capacity. Changing this policy increments `VOXEL_GENERATOR_CACHE_VERSION` so persistent payloads are regenerated without changing the emitter wire layout
- coarser payload emitters come from a persisted hierarchy of bounded LOD `1` source summaries aligned to LOD `1, 2, 4, 8, 16, 32` region footprints; each parent composes four children and retains additive RGB power, luminance-weighted position, source count, conservative exposure, and source extent. A coarse request composes its 3x3 same-LOD summary neighborhood and keeps representatives whose 14-28 block radius intersects its footprint, marking neighbor-owned records as payload-local halos so adjacent meshes bake from equivalent boundary sources without duplicate runtime accents
