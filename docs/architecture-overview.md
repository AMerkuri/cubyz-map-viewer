...
- the server builds LOD `1` halo records with generation-local external-region and unified target/external traversability-promise caches, then applies the cache-versioned deterministic 8,192-record selection boundary: when capped, 1,024 slots protect boundary-relevant halo sources (up to 256 nominations per eligible X/Y edge, with corner deduplication, 12-cell 3D/vertical relevance, global reservation fill, and deterministic own/halo fallback); changing this policy increments `VOXEL_GENERATOR_CACHE_VERSION` so persistent payloads are regenerated without changing the emitter wire layout
- coarser payload emitters come from a persisted hierarchy of bounded LOD `1` source summaries aligned to LOD `1, 2, 4, 8, 16, 32` region footprints; each parent composes four children and retains additive RGB power, luminance-weighted position, source count, conservative exposure, and source extent, so sources discarded by Cubyz same-LOD chunks remain eligible
...
- seam-affecting server changes cross this payload/cache/worker boundary and therefore require matrix evidence for X/Y edges, corners, vertical extremes, dense own and both-side pressure, and missing/special neighbors below and at cap pressure: decoded retained records must be deterministic, designated receiving geometry must retain non-zero fixed-camera baked light, and batching/traversal-only uncapped changes must preserve payload records while not regressing cache-miss halo timing
