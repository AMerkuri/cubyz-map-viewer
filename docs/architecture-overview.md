...
- voxel mesh payloads include a versioned compact emitter-record section after greedy/model geometry records; LOD `1` payloads may include region-local block emitter coordinates plus RGB bytes derived from Cubyz `.emittedLight`, while coarser LODs encode an empty emitter set until aggregation semantics are defined
