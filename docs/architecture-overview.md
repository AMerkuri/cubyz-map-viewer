...
- debug-only voxel-lighting diagnostics: the client may request `/api/voxels/...?halo=0` to receive a payload generated without neighboring-region halo emitter records; such diagnostic payloads are cached and ETagged separately from normal payloads, and voxel responses expose `X-Voxel-Cache`, current-generation `X-Voxel-Halo-Ms`, optional cached-generation `X-Voxel-Cached-Halo-Ms`, `X-Voxel-Own-Emitters`, and `X-Voxel-Halo-Emitters` headers so the client debug stats can compare cold server halo cost, warm cache serving, and client emissive bake cost; the client additionally derives worker-side emissive bake phase metrics (emissive grid-build time, emissive bake time, and emissive quads evaluated vs. conservatively culled) from `WorkerOut` to isolate client emissive cost from server generation, independent of any server payload or header change; cache-miss responses additionally expose aggregate external-region cache counters (`X-Voxel-Ext-Region-Parses`, `X-Voxel-Ext-Region-Cache-Hits`, `X-Voxel-Ext-Region-Misses`, `X-Voxel-Ext-Region-Parse-Errors`) from the LOD `1` generation-local external region cache so halo timing can be interpreted against neighboring source-load reuse; default behavior (no query parameter) is unchanged
