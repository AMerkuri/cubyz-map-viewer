...
- Persistent voxel mesh cache keys include `VOXEL_GENERATOR_CACHE_VERSION` and the block shape table signature. The shape table signature includes a semantic support version plus shape-affecting block definition/model inputs (including texture-pile model references and state counts), so semantic implementation or asset changes invalidate stale meshes. When the voxel generator changes geometry, traversal semantics, transparent classification, or binary payload semantics, `VOXEL_GENERATOR_CACHE_VERSION` is bumped so previously persisted meshes are regenerated rather than reused.
...
- `BlockShapeTable` scans `blocks/**/*.zig.zon`, applies inherited `_defaults.zig.zon` values by directory, resolves static `.model` references against `assets/*/models/*.obj`, and records palette-indexed cube, air, supported model, or supported semantic shapes. A direct `cubyz:cube` model reference is normalized back to the cube shape so cube-like transparent blocks can use greedy connected-face meshing. For `cubyz:texture_pile` it reads the nested `.model.model` reference as the base geometry and the `.model.states` count as finite state metadata.
