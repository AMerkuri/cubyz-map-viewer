...
- The voxel generator writes a cache-versioned binary payload. Greedy cube quads with integer cell boundaries use compact `u16` position coordinates relative to the response origin; model/semantic quads and other fractional geometry use `u32` fixed-point coordinates in `1/4096` voxel-cell units. This preserves supported OBJ block model vertices as authored block-local coordinates, including coordinates outside the `0..1` unit-block range, while keeping ordinary full-cube terrain on a compact path.
- The voxel generator writes a padded `u16` per-quad block palette index section after the winding flags, followed by padded `u8` render-kind, source-kind, and position-kind sections before vertex positions. Palette values resolve through `/api/world/block-palette`; out-of-range indices are encoded as `0xFFFF` so the client omits block identity instead of guessing. Render kind `1` is opaque and render kind `2` is transparent. Source kind distinguishes greedy cube quads from model/semantic quads for metrics.
- Voxel generation metrics distinguish total quads, greedy cube quads, model/semantic quads, dropped model quads, transparent quads, raw payload bytes, cache tier, chunk columns, parse/mesh work, and generated Z bounds. `VoxelMeshService` propagates these through the normal `/api/voxels` route and benchmark path; routes must not bypass the service to serve voxel payloads.
