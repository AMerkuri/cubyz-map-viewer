...
- Emitter records are generated only for LOD `1` blocks whose palette entry has non-zero `.emittedLight`. Each compact record stores response-local block coordinates plus RGB bytes decoded from `0xRRGGBB`; coarser LODs encode an empty set. The server does not infer block lights from `*_emission.png` texture filenames.
- Voxel generation metrics distinguish total quads, greedy cube quads, model/semantic quads, dropped model quads, transparent quads, raw payload bytes, greedy record bytes, model record bytes, emitter records/bytes, cache tier, chunk columns, parse/mesh work, and generated Z bounds. `VoxelMeshService` propagates these through the normal `/api/voxels` route and benchmark path; routes must not bypass the service to serve voxel payloads.
...
- Persistent voxel mesh cache keys include `VOXEL_GENERATOR_CACHE_VERSION`, the block shape table signature, and the block visual table signature. The shape table signature includes a semantic support version plus shape-affecting block definition/model inputs (including texture-pile model references, state counts, and supported OBJ coordinate interpretation), while the visual signature includes color/render/emitted-light metadata. When geometry, traversal semantics, transparent classification, emitted-light values, or binary payload semantics change, stale persisted meshes are regenerated rather than reused.
