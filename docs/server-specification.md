...
- Emitter records are generated only for LOD `1` blocks whose palette entry has non-zero `.emittedLight`. Each compact record stores signed response-relative block coordinates, RGB bytes decoded from `0xRRGGBB`, an internal halo flag, and an open-face mask derived from traversable neighboring cells. Fully enclosed emitters are omitted. Own-region records are emitted first, then neighboring-region halo records from the emitted-light radius-expanded border band, with deterministic coordinate ordering and a fixed per-payload record cap. Coarser LODs encode an empty set. The server does not infer block lights from `*_emission.png` texture filenames.
- Voxel generation metrics distinguish total quads, greedy cube quads, model/semantic quads, dropped model quads, transparent quads, raw payload bytes, greedy record bytes, model record bytes, total/own/halo emitter records and emitter bytes, cache tier, chunk columns, parse/mesh work, and generated Z bounds. `VoxelMeshService` propagates these through the normal `/api/voxels` route and benchmark path; routes must not bypass the service to serve voxel payloads.
...
- Persistent voxel mesh cache keys include `VOXEL_GENERATOR_CACHE_VERSION`, the block shape table signature, the block visual table signature, and for LOD `1` the neighboring halo column signatures that can affect the payload's emitted-light records. The shape table signature includes a semantic support version plus shape-affecting block definition/model inputs (including texture-pile model references, state counts, and supported OBJ coordinate interpretation), while the visual signature includes color/render/emitted-light metadata. When geometry, traversal semantics, transparent classification, emitted-light values, neighboring halo emitters, or binary payload semantics change, stale persisted meshes are regenerated rather than reused.
