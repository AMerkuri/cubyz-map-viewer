...
- The voxel generator writes shared padded sections for per-quad color, AO, winding, block palette index, and render kind, followed by greedy records and then model records. Palette values resolve through `/api/world/block-palette`; out-of-range indices are encoded as `0xFFFF` so the client omits block identity instead of guessing. Render kind `1` is opaque and render kind `2` is transparent. Header-level greedy/model record counts replace the previous per-quad source-kind and position-kind side arrays.
- Voxel generation metrics distinguish total quads, greedy cube quads, model/semantic quads, dropped model quads, transparent quads, raw payload bytes, greedy record bytes, model record bytes, cache tier, chunk columns, parse/mesh work, and generated Z bounds. `VoxelMeshService` propagates these through the normal `/api/voxels` route and benchmark path; routes must not bypass the service to serve voxel payloads.
