...
- The voxel worker reads the cache-versioned mixed position payload. Greedy cube quads can use compact `u16` integer cell coordinates, while fractional model/semantic quads use fixed-point `u32` X/Y/Z values in `1/4096` voxel-cell units. Both paths convert to world coordinates before the shared quadrant mesh, normal, color, bounds, and chunk-top-height builders run.
...
- The debug chunk stats separate loaded voxel geometry bytes from retained CPU metadata bytes, voxel warm-cache bytes, queued worker-output bytes, and memory by LOD. Geometry attributes are counted once; retained metadata covers only arrays still needed for AO refresh and hover identity.
