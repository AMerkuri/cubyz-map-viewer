...
import { parseRegionFile, CHUNK_SIZE, REGION_SIZE, type ChunkData, type RegionData } from "../parsers/region.js";
import { parseSurfaceFile, MAP_SIZE, type SurfaceData } from "../parsers/surface.js";
import { encodeBinaryQuads, VOXEL_REGION_SIZE, type BinaryQuad } from "./greedy-mesh.js";
...
const PROJECT_VOXEL_CACHE_DIR = resolve(process.cwd(), "dist", "server", "cache", "voxels");
...
export async function generateVoxelMesh(
  ...
  const surfaceSignature = await computeSurfaceHeightsWithSignature(savePath, regionX, regionY, lod);
  if (!surfaceSignature.data.hasSurface) return { buffer: null };

  const saveCacheNamespace = createHash("sha1").update(savePath).digest("hex").slice(0, 16);
  const persistentCacheDir = join(PROJECT_VOXEL_CACHE_DIR, saveCacheNamespace, String(lod), String(regionX));
  const persistentCachePath = join(persistentCacheDir, `${regionY}.bin`);
  const cacheKey = createHash("sha1")
    .update(`${VOXEL_GENERATOR_CACHE_VERSION}|${MAX_ENTRANCE_DEPTH_WORLD}|${columnSignature.signature}|${surfaceSignature.signature}|${lod}|${regionX}|${regionY}`)
    .digest("hex");
  ...
  const minChunkZ = Math.floor(Math.min(...columnSignature.zValues) / lod / CHUNK_SIZE);
  const maxChunkZ = Math.floor(Math.max(...columnSignature.zValues) / lod / CHUNK_SIZE) + REGION_SIZE - 1;
  ...
  for (const [key, typ] of faces) {
    const [xStr, yStr, zStr, face] = key.split("/") as [string, string, string, Direction];
    const x = parseInt(xStr);
    const y = parseInt(yStr);
    visibleChunkColumns.add(Math.floor(x / CHUNK_SIZE) * 4 + Math.floor(y / CHUNK_SIZE));
    void typ;
    void face;
  }
  ...
  const mesh = encodeBinaryQuads(quads, regionX, regionY, baseWorldZ, lod, blockColors, chunkCoverage);
  ...
  async function seedBoundaryColumn(gx: number, gy: number, dx = 0, dy = 0): Promise<void> {
    const startCell = Math.floor(surfaceSignature.data.heights[gx * COLUMN_VOXELS + gy] / lod) + 1;
    if (startCell < minChunkZ * CHUNK_SIZE || startCell > (maxChunkZ + 1) * CHUNK_SIZE - 1) return;
  ...
  async function seedWorldAir(gx: number, gy: number, startCell: number): Promise<void> {
  ...
  function seedChunkAir(chunkX: number, chunkY: number, chunkZ: number, lx: number, ly: number, lz: number): void {
    if (chunkX < 0 || chunkX >= CHUNK_COLUMNS_PER_AXIS || chunkY < 0 || chunkY >= CHUNK_COLUMNS_PER_AXIS) return;
    ...
    if (!isWithinEntranceDepth(gx, gy, gz, surfaceSignature.data.heights, lod)) return;
  ...
  async function processChunk(state: ChunkState): Promise<void> {
    ...
    while (queue.length > 0) {
      ...
      for (const step of FACE_STEPS) {
        ...
        if (nlx >= 0 && nlx < CHUNK_SIZE && nly >= 0 && nly < CHUNK_SIZE && nlz >= 0 && nlz < CHUNK_SIZE) {
          ...
          if (isAir(chunk, neighborIdx)) {
            if (state.visitedAir[neighborIdx] === 0) {
              ...
              if (isWithinEntranceDepth(ngx, ngy, ngz, surfaceSignature.data.heights, lod)) {
            ...
            if (shouldEmitFace(face, solidX, solidY, solidZ, surfaceSignature.data.heights, lod)) {
              addFace(faces, solidX, solidY, solidZ, face, getType(chunk, neighborIdx));
            }
          }
          continue;
        }

        const neighborChunkX = state.chunkX + (nlx < 0 ? -1 : nlx >= CHUNK_SIZE ? 1 : 0);
        const neighborChunkY = state.chunkY + (nly < 0 ? -1 : nly >= CHUNK_SIZE ? 1 : 0);
        const neighborChunkZ = state.chunkZ + (nlz < 0 ? -1 : nlz >= CHUNK_SIZE ? 1 : 0);
        ...
        if (neighborChunkX < 0 || neighborChunkX >= CHUNK_COLUMNS_PER_AXIS || neighborChunkY < 0 || neighborChunkY >= CHUNK_COLUMNS_PER_AXIS) {
          continue;
        }
        if (neighborChunkZ < minChunkZ || neighborChunkZ > maxChunkZ) {
          continue;
        }
        const neighborChunk = await loadChunk(neighborChunkX, neighborChunkY, neighborChunkZ);
        if (!neighborChunk) {
          continue;
        }
        const wrappedIdx = localIndex(wrappedLx, wrappedLy, wrappedLz);
        if (isAir(neighborChunk, wrappedIdx)) {
          seedChunkAir(neighborChunkX, neighborChunkY, neighborChunkZ, wrappedLx, wrappedLy, wrappedLz);
          continue;
        }
        const face = OPPOSITE_FACE[step.face];
        const solidX = gx + step.dx;
        ...
        const solidZ = gz + step.dz;
        if (shouldEmitFace(face, solidX, solidY, solidZ, surfaceSignature.data.heights, lod)) {
          addFace(faces, solidX, solidY, solidZ, face, getType(neighborChunk, wrappedIdx));
        }
      }
    }
  }

  function getChunkState(chunkX: number, chunkY: number, chunkZ: number): ChunkState {
  ...
  async function loadChunk(chunkX: number, chunkY: number, chunkZ: number): Promise<ChunkData | null> {
    ...
    if (!loader) {
      loader = (async () => {
        ...
        try {
          regionsParsed++;
          return await parseRegionFile(path, regionX, regionY, regionWorldZ, lod);
        } catch (error) {
    ...
    const chunkIndex = chunkX * REGION_SIZE * REGION_SIZE + chunkY * REGION_SIZE + localChunkZ;
  ...
  async function emitOuterRegionBoundaryFaces(): Promise<void> {
    for (let chunkZ = minChunkZ; chunkZ <= maxChunkZ; chunkZ++) {
      for (let chunkY = 0; chunkY < CHUNK_COLUMNS_PER_AXIS; chunkY++) {
        ...
        const rightChunk = await loadChunk(CHUNK_COLUMNS_PER_AXIS - 1, chunkY, chunkZ);
        if (rightChunk) {
          await emitChunkEdgeFaces(rightChunk, CHUNK_COLUMNS_PER_AXIS - 1, chunkY, chunkZ, CHUNK_SIZE - 1, "x+");
        }
      }
      for (let chunkX = 0; chunkX < CHUNK_COLUMNS_PER_AXIS; chunkX++) {
        ...
        const backChunk = await loadChunk(chunkX, CHUNK_COLUMNS_PER_AXIS - 1, chunkZ);
        if (backChunk) {
          await emitChunkEdgeFaces(backChunk, chunkX, CHUNK_COLUMNS_PER_AXIS - 1, chunkZ, CHUNK_SIZE - 1, "y+");
        }
      }
    }
  }
  ...
  async function emitChunkEdgeFaces(
    ...
    const neighborChunk = await loadExternalChunk(edgeChunkX, edgeChunkY, chunkZ);
    ...
    for (let u = 0; u < CHUNK_SIZE; u++) {
      for (let v = 0; v < CHUNK_SIZE; v++) {
        ...
        if (!isBoundaryVisibleFromAir(face, gx, gy, gz, airX, airY, surfaceSignature.data.heights, lod)) continue;

        const neighborLocalX = face === "x-" ? CHUNK_SIZE - 1 : face === "x+" ? 0 : lx;
        const neighborLocalY = face === "y-" ? CHUNK_SIZE - 1 : face === "y+" ? 0 : ly;
  ...
  async function loadExternalChunk(chunkX: number, chunkY: number, chunkZ: number): Promise<ChunkData | null> {
    const externalRegionX = regionX + chunkX * CHUNK_SIZE * lod;
    const externalRegionY = regionY + chunkY * CHUNK_SIZE * lod;
    const columnWorldSpan = COLUMN_VOXELS * lod;
    const normalizedRegionX = Math.floor(externalRegionX / columnWorldSpan) * columnWorldSpan;
    const normalizedRegionY = Math.floor(externalRegionY / columnWorldSpan) * columnWorldSpan;
    const localChunkX = Math.floor((externalRegionX - normalizedRegionX) / (CHUNK_SIZE * lod));
    const localChunkY = Math.floor((externalRegionY - normalizedRegionY) / (CHUNK_SIZE * lod));
    const regionChunkZ = Math.floor(chunkZ / REGION_SIZE) * REGION_SIZE;
    const regionWorldZ = regionChunkZ * CHUNK_SIZE * lod;
    const path = join(savePath, "chunks", String(lod), String(normalizedRegionX), String(normalizedRegionY), `${regionWorldZ}.region`);
    if (!existsSync(path)) return null;
    try {
      const region = await parseRegionFile(path, normalizedRegionX, normalizedRegionY, regionWorldZ, lod);
      const localChunkZ = chunkZ - regionChunkZ;
      const chunkIndex = localChunkX * REGION_SIZE * REGION_SIZE + localChunkY * REGION_SIZE + localChunkZ;
...
async function buildColumnSignature(colDir: string): Promise<ColumnSignature> {
  ...
  const zValues = entries
    ...
    .map((entry) => parseInt(entry.slice(0, -".region".length)))
...
async function computeSurfaceHeightsWithSignature(
  ...
  function getSurfaceTile(tileX: number, tileY: number): Promise<SurfaceData | null> {
    ...
    for (let tileY = tileYStart; tileY <= tileYEnd; tileY += MAP_SIZE) {
      ...
      for (let wx = overlapX0; wx < overlapX1; wx++) {
        for (let wy = overlapY0; wy < overlapY1; wy++) {
          ...
          if (localX < 0 || localX >= COLUMN_VOXELS || localY < 0 || localY >= COLUMN_VOXELS) continue;
  ...
  if (!foundSurface || !isFinite(minSurfaceH)) {
    return {
      signature: hash.digest("hex"),
      data: { heights: new Int32Array(COLUMN_VOXELS * COLUMN_VOXELS), minHeight: 0, hasSurface: false },
    };
  }
...
function isVisibleSolid(x: number, y: number, z: number, surfaceHeights: Int32Array, lod: number): boolean {
  if (x < 0 || x >= COLUMN_VOXELS || y < 0 || y >= COLUMN_VOXELS) return false;
  const worldCellTopZ = (z + 1) * lod;
  return worldCellTopZ > surfaceHeights[x * COLUMN_VOXELS + y] - MAX_ENTRANCE_DEPTH_WORLD;
}

function shouldEmitFace(face: Direction, x: number, y: number, z: number, surfaceHeights: Int32Array, lod: number): boolean {
...
function isBoundaryVisibleFromAir(
  ...
  if (surfaceX < 0 || surfaceX >= COLUMN_VOXELS || surfaceY < 0 || surfaceY >= COLUMN_VOXELS) {
    return false;
  }
  const worldCellBottomZ = solidZ * lod;
  return worldCellBottomZ >= surfaceHeights[surfaceX * COLUMN_VOXELS + surfaceY] - MAX_ENTRANCE_DEPTH_WORLD;
}

function isWithinEntranceDepth(x: number, y: number, z: number, surfaceHeights: Int32Array, lod: number): boolean {
  if (x < 0 || x >= COLUMN_VOXELS || y < 0 || y >= COLUMN_VOXELS) return false;
  const worldCellBottomZ = z * lod;
  return worldCellBottomZ >= surfaceHeights[x * COLUMN_VOXELS + y] - MAX_ENTRANCE_DEPTH_WORLD;
}

function addFace(faceMap: Map<string, number>, x: number, y: number, z: number, face: Direction, typ: number): void {
...
function buildMergedQuads(faceMap: Map<string, number>): BinaryQuad[] {
  ...
  for (const [planeKey, group] of groups) {
    ...
    for (const u of uValues) {
      ...
      const isSideFace = face === "x+" || face === "x-" || face === "y+" || face === "y-";
      for (const v of vValues) {
        ...
        const maxV = face === "z-" ? Math.min(COLUMN_VOXELS, v + cellsUntilChunkBoundary(v)) : Number.MAX_SAFE_INTEGER;
        let dv = 1;
...
function getPlaneCoordinate(x: number, y: number, z: number, face: Direction): number {
  switch (face) {
    case "x+": return x + 1;
    case "x-": return x;
    case "y+": return y + 1;
...
function getPlaneAxes(x: number, y: number, z: number, face: Direction): { u: number; v: number } {
  switch (face) {
    case "x+":
    case "x-":
      return { u: y, v: z };
...
function createMergedQuad(face: Direction, plane: number, u: number, v: number, du: number, dv: number, typ: number): BinaryQuad {
  switch (face) {
    case "x+":
      return { v0x: plane, v0y: u, v0z: v, v1x: plane, v1y: u + du, v1z: v, v2x: plane, v2y: u + du, v2z: v + dv, v3x: plane, v3y: u, v3z: v + dv, typ, dir: 1 };
    case "x-":
      return { v0x: plane, v0y: u, v0z: v, v1x: plane, v1y: u + du, v1z: v, v2x: plane, v2y: u + du, v2z: v + dv, v3x: plane, v3y: u, v3z: v + dv, typ, dir: -1 };
    case "y+":
      return { v0x: u, v0y: plane, v0z: v, v1x: u + du, v1y: plane, v1z: v, v2x: u + du, v2y: plane, v2z: v + dv, v3x: u, v3y: plane, v3z: v + dv, typ, dir: -1 };
    case "y-":
      return { v0x: u, v0y: plane, v0z: v, v1x: u + du, v1y: plane, v1z: v, v2x: u + du, v2y: plane, v2z: v + dv, v3x: u, v3y: plane, v3z: v + dv, typ, dir: 1 };
    case "z+":
      return { v0x: u, v0y: v, v0z: plane, v1x: u + du, v1y: v, v1z: plane, v2x: u + du, v2y: v + dv, v2z: plane, v3x: u, v3y: v + dv, v3z: plane, typ, dir: 1 };
    case "z-":
      return { v0x: u, v0y: v, v0z: plane, v1x: u + du, v1y: v, v1z: plane, v2x: u + du, v2y: v + dv, v2z: plane, v3x: u, v3y: v + dv, v3z: plane, typ, dir: -1 };
  }
}
...
function createQuadVertices(x: number, y: number, z: number, face: Direction): Omit<BinaryQuad, "typ" | "dir"> {
  switch (face) {
    case "x+":
      return { v0x: x + 1, v0y: y, v0z: z, v1x: x + 1, v1y: y + 1, v1z: z, v2x: x + 1, v2y: y + 1, v2z: z + 1, v3x: x + 1, v3y: y, v3z: z + 1 };
    case "x-":
      return { v0x: x, v0y: y, v0z: z, v1x: x, v1y: y + 1, v1z: z, v2x: x, v2y: y + 1, v2z: z + 1, v3x: x, v3y: y, v3z: z + 1 };
    case "y+":
      return { v0x: x, v0y: y + 1, v0z: z, v1x: x + 1, v1y: y + 1, v1z: z, v2x: x + 1, v2y: y + 1, v2z: z + 1, v3x: x, v3y: y + 1, v3z: z + 1 };
    case "y-":
      return { v0x: x, v0y: y, v0z: z, v1x: x + 1, v1y: y, v1z: z, v2x: x + 1, v2y: y, v2z: z + 1, v3x: x, v3y: y, v3z: z + 1 };
    case "z+":
      return { v0x: x, v0y: y, v0z: z + 1, v1x: x + 1, v1y: y, v1z: z + 1, v2x: x + 1, v2y: y + 1, v2z: z + 1, v3x: x, v3y: y + 1, v3z: z + 1 };
    case "z-":
      return { v0x: x, v0y: y, v0z: z, v1x: x + 1, v1y: y, v1z: z, v2x: x + 1, v2y: y + 1, v2z: z, v3x: x, v3y: y + 1, v3z: z };
  }
}

async function readPersistentMesh(cachePath: string, expectedKey: string): Promise<ArrayBuffer | null> {
  ...
  try {
    ...
    return mesh.buffer.slice(mesh.byteOffset, mesh.byteOffset + mesh.byteLength);
  } catch {
    return null;
  }
}

async function writePersistentMesh(cachePath: string, key: string, mesh: ArrayBuffer): Promise<void> {
  const keyBuffer = Buffer.from(key, "utf8");
  const header = Buffer.allocUnsafe(2);
  header.writeUInt16LE(keyBuffer.length, 0);
  await writeFile(cachePath, Buffer.concat([header, keyBuffer, Buffer.from(mesh)]));
}
...
function extractMaxWorldZ(view: DataView, byteLength: number): number {
